  ...
  return (
    <div className="w-full max-w-md mx-auto space-y-8 relative group/search">
      <div className={`flex items-center gap-4 ${isAr ? "flex-row-reverse" : ""}`}>
        <div className="w-12 h-12 rounded-[20px] bg-blue-500/10 border border-blue-500/20 flex items-center justify-center shadow-[0_0_20px_rgba(59,130,246,0.1)]">
        ...
        <div className={isAr ? "text-right" : ""}>
          ...
          <p className="text-blue-500/40 text-[8px] font-black uppercase tracking-[0.4em] mt-1">
      ...
      <div className="relative group">
        <Search
          className={`absolute top-1/2 -translate-y-1/2 w-4 h-4 text-white/10 group-focus-within:text-blue-400 transition-colors ${isAr ? "right-6" : "left-6"}`}
        />
        <input
          type="text"
          value={query}
          ...
          className={`w-full bg-black/40 border border-white/5 rounded-[24px] py-5 text-white font-bold placeholder:text-white/5 focus:outline-none focus:border-blue-500/30 focus:bg-white/[0.04] transition-all shadow-2xl ${isAr ? "pr-14 text-right" : "pl-14"}`}
        />
        {loading && (
          <div className={`absolute top-1/2 -translate-y-1/2 ${isAr ? "left-6" : "right-6"}`}>
        ...
        <AnimatePresence mode="popLayout">
          {results.map((res) => (
            <motion.div
              ...
              className={`flex items-center justify-between p-6 rounded-[32px] bg-white/[0.02] border border-white/5 backdrop-blur-3xl hover:bg-white/[0.05] hover:border-blue-500/30 transition-all duration-500 shadow-xl group/res ${isAr ? "flex-row-reverse" : ""}`}
            >
              <div className={`flex items-center gap-4 ${isAr ? "flex-row-reverse" : ""}`}>
                <div className="w-12 h-12 rounded-2xl overflow-hidden bg-black/40 border border-white/10 flex-shrink-0 relative group-hover/res:border-blue-500/50 transition-colors duration-500">
                  {res.avatar_url ? (
                    <img
                      src={res.avatar_url}
                      alt=""
                ...
                <div className={isAr ? "text-right" : ""}>
                  ...
                  <div className="text-[8px] text-blue-500/40 font-black uppercase tracking-[0.3em] mt-1">
              ...
              <HeroButton
                ...
                variant="secondary"
                size="md"
                className="px-6 py-3 rounded-xl shadow-blue-500/10"
              >
                {invitingId === res.id ? (
