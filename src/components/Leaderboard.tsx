  ...
  return (
    <div className="w-full max-w-md mx-auto space-y-4 relative z-10">
      <div className={`flex items-center gap-3 mb-6 ${isAr ? "flex-row-reverse" : ""}`}>
        ...
        <h3 className="text-xl font-bold text-white tracking-tight uppercase italic">{t.title}</h3>
      </div>
          ...
          >
            <div className={`flex items-center gap-3 ${isAr ? "flex-row-reverse" : ""}`}>
              ...
              <div className="flex flex-col">
                <span className={`font-bold text-white text-sm ${isAr ? "text-right" : ""}`}>
                  {p.username}
                </span>
                <div className={`flex items-center gap-1 ${isAr ? "flex-row-reverse" : ""}`}>
                  <span className="text-blue-400 font-black text-xs leading-none">{p.score}</span>
