...
function LevelClassroomPage() {
  console.log("Classroom component mounting...");
  const { levelId } = Route.useSearch<{ levelId: string }>();
  ...
  const fetchLevelDetailsAndAccess = useCallback(async () => {
    ...
    console.log(`Starting access check for level: ${levelId}`);
    setLoading(true);
    try {
      // Fetch level details
      ...
      if (isAdmin || isModerator) {
        ...
        if (accessData) {
          ...
          // Check sequential progression via RPC
          const { data: canAccess, error: rpcError } = await supabase.rpc("can_student_access_level", {
            u_id: user.id,
      ...
      if (!accessGranted) {
        console.log(`Access denied for ${levelId}, redirecting to /levels`);
  ...
  }, [levelId, user, isApproved, isAdmin, isModerator, navigate]);
